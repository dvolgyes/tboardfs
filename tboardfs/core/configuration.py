...
class ExportFormatConfig:
    ...
    def validate(self) -> None:
        ...
        if self.image_format not in {ImageFormats.PNG, ImageFormats.JPG, ImageFormats.JPEG, ImageFormats.GIF}:
        ...
        if not (AudioFormats.MIN_SAMPLE_RATE <= self.audio_sample_rate <= AudioFormats.MAX_SAMPLE_RATE):
            raise InvalidConfigurationError(
                "audio_sample_rate",
        ...
        if self.ply_format not in {PLYFormats.BINARY, PLYFormats.TEXT, PLYFormats.ASCII}:
            raise InvalidConfigurationError(
                "ply_format",
                self.ply_format,
...
class TBoardFSConfig:
    ...
    output_organization: OutputOrganizationConfig = field(default_factory=OutputOrganizationConfig)
